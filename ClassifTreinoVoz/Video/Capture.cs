...
namespace DxPlay
{
    public class DxPlay : IDisposable
    {
        ...
        /// <summary>Attempts to extract audio from file</summary>
        /// <param name="file">File to extract audio from</param>
        public static string ExtractAudioFromVideo(string file)
        {
            try
            {
                ...
                if (File.Exists(Application.StartupPath + "\\ffmpeg.exe"))
                {
                    ...
                    string dstFile = Application.StartupPath + "\\tmpAud"+rnd.Next(10000,99999)+".wav";
                    if (File.Exists(dstFile)) File.Delete(dstFile);

                    string args = "-i \"" + file + "\" \"" + dstFile + "\"";
                    Process proc = System.Diagnostics.Process.Start(Application.StartupPath + "\\ffmpeg.exe", args);
                    proc.WaitForExit();
                    return dstFile;
                }
                else
        ...
        public void SetWindowSize(Control hWin)
        {
            int hr;
        ...
        public void SetTime(double time)
        {
            ...
            IMediaPosition imp = m_FilterGraph as IMediaPosition;
            hr = imp.put_CurrentPosition(time);
        }
